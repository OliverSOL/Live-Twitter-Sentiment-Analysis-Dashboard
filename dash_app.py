...
app.layout = html.Div(
	children=[
		...
		html.Div(className='row',
			children=[
			# -------------------left side of app--------------------#
			html.Div(className='seven columns div-user-controls',
				children=[
				
				html.H2('Amazon Analysis'),
				html.P('''A deep dive into what twitter users are saying about Amazon in real time.'''),
				#-------------graph-----------------#
				html.Div([
					dcc.Graph(
						id='live-graph',
...
def update_pie(interval):
	try:
		...
		data = go.Pie(labels=labels, values=vals_sent, hole=.5, pull=[0,0,0])
		layout = go.Layout(
						autosize=True,
...
def update_tweet(tweets):
	...
	'''if len(tweetdf) > 500:
					tweetdf.c.excecute("DELETE FROM AmazonTweets")
					tweetdf.conn.commit()'''
